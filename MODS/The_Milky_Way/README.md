**_TLDR: "Add Prolactin Amplification to the Cattle mutation branch, allowing you to milk NPCs once a day with a bucket in your inventory". Mod options in main.lua_**
...
- What if I find the messages immersion breaking?
  - open `main.lua` with any text editor, and change `CORPORATE_MILK = false` to `CORPORATE_MILK = true`
  - this will make the message less humorous and more professional
...
2) If you milk a NPC (milking a human appears), but don't get milk
    - Solution: open `main.lua` with any text editor, and increase the number next to `MILK_ACTIVITY_MARGIN = `.
    - Theoretically it should never happen, but there might be some strange interactions with other mods that causes this, so I created this system just in case
