...
-- monsters that can spawn depending on effect intensity: be careful that they are not from an in-repo mod!
local real_spawns = {
	{ "mon_blob_small" }, -- 0
...
function mod.on_character_effect(params) -- effect, char
	...
	-- we don't care about potential other effects that use EFFECT_LUA_ON_TICK
	...
	if math.random(0, 5) == 0 then -- 16% of the time, we spawn something
		-- we want to avoid monsters spawning too close, but too far makes their spawn useless (many monsters are near-sighted in BN?)
