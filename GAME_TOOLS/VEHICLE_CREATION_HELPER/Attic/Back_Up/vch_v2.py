...
# Flag to know if we're on the install vehicle part screen, or not. TODO remove if not used in final version
# ON_INSTALL_SCREEN_TEXT = "Safe/Top Speed"
...
# STRUCTS
# Options reprentation, extracted from the options.json file
...
def too_many_misplaced_characters(text, subline, max_diff_allowed):
	...
	# TODO remove print_and_log(str(misplaced_character_count))
...
def update_vhc_updated_file():
	...
	new_vhc_file = pathlib.Path(VCH_UPDATED_FILE_NAME)
...
def install_vehicle_part(vehicle_part_name):
	...
	if not check_if_texts_are_on_screen_location([vehicle_part_name], CURRENT_SCREENSHOT.width/3, 0, 2*CURRENT_SCREENSHOT.width/3, CURRENT_SCREENSHOT.height/4, True, 120):
	...
	while not is_vehicle_part_installed(vehicle_part_name, try_psm):
		...
		# TODO since the function is better optimised, add an other check later? Eg every 10 sec?
...
try:
	...
	OPTIONS = get_objects_from_file(pathlib.Path(OPTIONS_FILE_NAME) if pathlib.Path(OPTIONS_FILE_NAME).exists() else pathlib.Path(f"../{OPTIONS_FILE_NAME}"), DataType.OPTIONS)[0]
	...
	VEHICLE_PARTS_DATA = get_objects_from_file(pathlib.Path(VCH_UPDATED_FILE_NAME if (OPTIONS.use_save_system and pathlib.Path(VCH_UPDATED_FILE_NAME).exists()) else VCH_FILE_NAME), DataType.VEHICLE_PART)
	...
	for vpd in VEHICLE_PARTS_DATA:
		...
		# Get the lowest row with a part to install
		max_row:int = 0
		for vr in range(len(vpd.vmap)):
			for vc in range(len(vpd.vmap[vr])):
				if vpd.vmap[vr][vc] == "X":
					max_row = vr
		# Iterate through all vehicle parts
		for vehicle_row in range(max_row+1):
			# Only navigates up to the furthest part to install on the right
			max_col:int = 0
			for vc in range(len(vpd.vmap[vehicle_row])):
				if vpd.vmap[vehicle_row][vc] == "X":
					max_col = vc
			if max_col != 0:
				for vehicle_col in range(max_col+1):
					...
					if vpd.vmap[vehicle_row][vehicle_col] == "X":
						...
						if not is_vehicle_part_installed(vpd.part_name):
		...
		for v in range(max_row+1):
			send_key(Key.up)

	exit_with_msg("Script finished!")

except:
	# Can be normal, safe exit and script errors. Everything is logged in the debug.txt file the same
