...
import random
import sys
import platform # to display current OS info
import traceback
import inspect
import pyperclip # we'll use the clipboard to send whole sentences instantly


# GLOBALS (Uppercase doesn't mean I'm screaming at you, it's just a global variable)
# Get keyboard
KB = keyboard.Controller()
# The file that will be used to build the vehicle
VCH_FILE_NAME = "vch_edit_me.json"
# The generated file that stores the parts that have been installed (or not)
VCH_UPDATED_FILE_NAME = "vch_parts_left_to_install.json"
# Options file name
...
CURRENT_SCREENSHOT = {}
# CURRENT_SCREENSHOT.show() DEBUG HERE, first check if the screenshot is correct (melting face smiley)
...
# STRUCTS
# Represent the custom user controls (cf options.json)
class Keys(Struct):
	ffilter: str
	install: str
	label: str
...
# The left, top, right and bottom delimitations of where the screenshot will be taken (in pixels) 
# Eg: you have 2 screens with 1920x1080 resolution and play on your first screen -> 0 0 1920, 1080. If you play on the 2nd -> 1920, 0, 3840, 1080
class ScreenInfo(Struct):
	left: int
	top: int
	right: int
	bottom: int

# Text flags the script detects
class Flags(Struct):
	# Flag to confirm if we want to make a vehicle unfoldable
	unfoldable: list
	# Flag to know if a part can't be installed on a tile (usually there's a wall or something)
	install_possible: list
	# Flags to know if there's something that could be a danger (close monster, pain...)
	danger: list
...
class OptionsData(Struct):
	...
	keys: Keys
	# cf class Flags(Struct)
	flags: Flags
	# cf class ScreenInfo(Struct)
	screen: ScreenInfo
	# To help me debug, 1 print a summary of what has happened, 2 print how the script is doing things
	debug_level: int
	# If true, show a screenshot of the area used for text recognition (should capture the whole game), then close the script
...
def set_clipboard_text(text):
    """Set new text into the clipboard."""
    try:
        pyperclip.copy(text)
    except pyperclip.PyperclipException as e:
        print_and_log(f"Failed to update clipboard: {e}")
...
def check_if_texts_are_on_screen_location(texts, left, top, right, bottom, do_update, threshold=10, try_psm=True):
	...
	# I had a problem by sending a string that was interpreted as an array of letters, instead of sending an array with 1 string inside
	# This was because I thought I could send a string as is, so now we have this violent JAVA style check that will ruthlessly stop the script in case that happens again
	if type(texts) != list:
		exit_with_msg("check_if_texts_are_on_screen_location -> WRONG texts type: " + str(type(texts)) + " instead of list!\ntexts was: " + str(texts))
	...
	for text in texts:
		...
		if OPTIONS.debug_level > 2:
			print_and_log("--- Targets: " + str(texts) + " --- Detected: " + str(lines))
			...
			while end_char_nb <= len(line):
				...
				if are_frequencies_roughly_similar(text_frequencies, line_frequencies, max_diff_allowed) and are_frequencies_roughly_similar(line_frequencies, text_frequencies, max_diff_allowed):
					...
					if not too_many_misplaced_characters(text, subline, max(max_diff_allowed, 1)) and not too_many_misplaced_characters(subline, text, max(max_diff_allowed, 1)):
						if OPTIONS.debug_level > 2:
							print_and_log(f"--- MATCH --> T: {text} VS D: {line}")
						return True
				start_char_nb += 1
				end_char_nb += 1
		if OPTIONS.debug_level > 2:
			print_and_log("--- No match for " + str(texts))
...
def update_screenshot(threshold=70, reverse=True):
	...
	time.sleep(0.03*OPTIONS.script_speed)
	CURRENT_SCREENSHOT = PIL.ImageGrab.grab(bbox=(OPTIONS.screen.left, OPTIONS.screen.top, OPTIONS.screen.right, OPTIONS.screen.bottom)).convert('L').point(fn, mode='1').convert('1')
	# CURRENT_SCREENSHOT.show() # DEBUG see the resulting image for the text recognition
...
def send_key(key):
	...
	if str(key).isupper():
		# That's how you send uppercase keys properly with pynput, same for alt, ctrl, shift...
	...
	if OPTIONS.debug_level > 1:
		print_and_log("-- Sent key: " + str(key))
...
def search_filter(text):
	...
	# To send the text fast & furious, we replace clipboard content and press CTRL + V
	set_clipboard_text(text)

	if OPTIONS.debug_level > 1:
		print_and_log("-- Sent text: " + str(text))
	with KB.pressed(Key.ctrl):
		send_key("v")
	send_key(Key.enter)

# Stop script if alt is pressed. Is executed each time a key is pressed
...
def process_vehicle_parts(raw_vehicle_parts_data):
	...
	for vpd in raw_vehicle_parts_data:
		...
		for vehicle_row in range(len(vpd.vmap)):
			...
			for vehicle_col in range(len(vpd.vmap[vehicle_row])):
				# Important! Replace "" by " " to avoid script error later (it would break vch_updated.json)
				# And now users can use "" instead of " " if they want, 2 birds one stone!
...
def exit_with_msg(msg):
	global SAFE_EXIT
	print_and_log(msg)
	set_clipboard_text("") # Empty clipboard content to avoid leaving vehicle part name garbage in it
...
def install_vehicle_part(vehicle_part_name, vehicle_col, vehicle_row):
	global EXIT_SCRIPT
	global OPTIONS
	# Open install menu
	send_key(OPTIONS.keys.install)
	# Check if we can construct a part here, if that's not the case, early return (eg: nothing can be installed on top of a ram, or a tile of air not connected to the vehicle)
	if not check_if_texts_are_on_screen_location(OPTIONS.flags.install_possible, CURRENT_SCREENSHOT.width/2, 0, CURRENT_SCREENSHOT.width, CURRENT_SCREENSHOT.height/2, True):
		print_and_log(f"Part skipped: {vehicle_part_name} at {vehicle_col+1}, {vehicle_row+1} - no part can be installed here")
		return False

	search_filter(vehicle_part_name)

	time.sleep(0.05*OPTIONS.script_speed)
	...
	if not check_if_texts_are_on_screen_location([vehicle_part_name], CURRENT_SCREENSHOT.width/3, 0, 2*CURRENT_SCREENSHOT.width/3, CURRENT_SCREENSHOT.height/4, True, 230):
		print_and_log(f"Part skipped: {vehicle_part_name} at {vehicle_col+1}, {vehicle_row+1} - part slot already taken or we don't have the materials")
		send_key(Key.esc)
		return False
	...
	# Filter red text only in the upper right part of the screen, if we find a line > 2 character, one the the req isn't met, skip this part
	...
	for vpart in vparts_choices:
		...
		if len(vpart) > 2:
			print_and_log(f"Part skipped: {vehicle_part_name} at {vehicle_col+1}, {vehicle_row+1} - an additional requirement is missing (red text)")
			send_key(Key.esc)
			return False

	# Start installation
	send_key(Key.enter)
	# Check if the game ask if we want to make the vehicle unfoldable, accept it
	if check_if_texts_are_on_screen_location(OPTIONS.flags.unfoldable, CURRENT_SCREENSHOT.width/5, CURRENT_SCREENSHOT.height/4, CURRENT_SCREENSHOT.width*4/5, CURRENT_SCREENSHOT.height*3/4, True, 100):
		send_key(OPTIONS.keys.confirm)
		if OPTIONS.debug_level > 0:
			print_and_log(f"- This part made the vehicle unfoldable {vehicle_part_name} at {vehicle_col+1}, {vehicle_row+1}")
	...
	while not is_vehicle_part_installed(vehicle_part_name, try_psm):
		# Check if the game needs us to select which component to use (eg: on us or nearby)
		if check_if_texts_are_on_screen_location(OPTIONS.flags.select_component, CURRENT_SCREENSHOT.width/3, CURRENT_SCREENSHOT.height/3, CURRENT_SCREENSHOT.width*2/3, CURRENT_SCREENSHOT.height*2/3, True, 150):
			if OPTIONS.debug_level > 2:
				print_and_log("--- Component selection menu detected")
			send_key(Key.enter)
			

		if EXIT_SCRIPT:
			exit_with_msg("Safe exit: user pressed the exit key, stopping the script")
		if time_waited > OPTIONS.max_action_wait:
			exit_with_msg("Safe exit: for unknown reasons, the vehicle part couldn't be installed")

		if time_waited > 5:
			# Check for warning messages (center of the screen)
			if check_if_texts_are_on_screen_location(OPTIONS.flags.danger, CURRENT_SCREENSHOT.width/5, CURRENT_SCREENSHOT.height/4, CURRENT_SCREENSHOT.width*4/5, CURRENT_SCREENSHOT.height*3/4, False):
				exit_with_msg("Safe exit: warning message detected! Could be dangerous, stopping the script")	
			# Sending enter just in case, the game might be laggy and we need to 
			send_key(Key.enter)
		...
		if len(OPTIONS.custom_script_stop_flags) > 0 and time_waited == 0 and INSTALLED_PARTS_COUNT % 3 == 0:
			if check_if_texts_are_on_screen_location(OPTIONS.custom_script_stop_flags, CURRENT_SCREENSHOT.width*2/3, 0, CURRENT_SCREENSHOT.width, CURRENT_SCREENSHOT.height/2, True):
				exit_with_msg("Safe exit: custom script stop flag detected! Stopping the script")

		try_psm = not try_psm

		time.sleep(0.2*OPTIONS.script_speed)
		time_waited += (0.2*OPTIONS.script_speed)
	return True
...
def process_cursor_move(current_cursor:Cursor, next_goal:Cursor):
	while(current_cursor.x != next_goal.x or current_cursor.y != next_goal.y):
		if OPTIONS.debug_level > 1:	
			print_and_log(f"-- Cur Cursor: {current_cursor.x+1}, {current_cursor.y+1}; Goal: {next_goal.x+1}, {next_goal.y+1}")

		if current_cursor.x < next_goal.x:
			send_key(Key.right)
			current_cursor.x += 1
		elif current_cursor.x > next_goal.x:
			send_key(Key.left)
			current_cursor.x -= 1
		elif current_cursor.y < next_goal.y:
			send_key(Key.down)
			current_cursor.y += 1
		elif current_cursor.y > next_goal.y:
			send_key(Key.up)
...
try:
	...
	OPTIONS.script_speed = 1/OPTIONS.script_speed # I thought it was weird to increase the option value to get a slower speed, so here we are
	set_move_keys()
	...
	update_screenshot(10)

	if OPTIONS.debug_screenshot:
		PIL.ImageGrab.grab(bbox=(OPTIONS.screen.left, OPTIONS.screen.top, OPTIONS.screen.right, OPTIONS.screen.bottom)).show()
		exit_with_msg("Safe exit: debug_screenshot")
	...
	if OPTIONS.debug_level > 0:
		print_and_log("- OS Info: "+ platform.platform())
		print_and_log("- Options: "+ str(OPTIONS))
	...
	for vpd in VEHICLE_PARTS_DATA:
		...
		for vehicle_row in range(len(vpd.vmap)):
			for vehicle_col in range(len(vpd.vmap[vehicle_row])):
				...
				if EXIT_SCRIPT:
					exit_with_msg("Safe exit: user pressed the exit key")
				...
				if not (is_vehicle_part_installed(vpd.part_name) or is_vehicle_part_installed(vpd.part_name, True)):
					...
					if install_vehicle_part(vpd.part_name, vehicle_col, vehicle_row):
						vpd.vmap[vehicle_row][vehicle_col] = "D"
						INSTALLED_PARTS_COUNT += 1
						if OPTIONS.debug_level > 0:
							print_and_log(f'- New part: {vpd.part_name} at {vehicle_col+1}, {vehicle_row+1}')
				# If the part is already installed without the script, mark it as installed
				else:
					vpd.vmap[vehicle_row][vehicle_col] = "D"
					if OPTIONS.debug_level > 0:
						print_and_log(f'- Already installed: {vpd.part_name} at {vehicle_col+1}, {vehicle_row+1}')

	# Show that the script is fully finished
	time.sleep(0.5)
	send_key(Key.esc)
	exit_with_msg("Safe exit: script fully finished")
